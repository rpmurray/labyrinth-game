...
class CharcterEntityBuilder {
    ...
    private static Printer PRINTER = PrinterFactory.getInstance().createPrinter(CharcterEntityBuilder.class);

    private static final String CHARACTER_RESOURCES_BACK = "RESOURCE_BACK";
    private static final String CHARACTER_RESOURCES_LEFT = "RESOURCE_LEFT";
    private static final String CHARACTER_RESOURCES_RIGHT = "RESOURCE_RIGHT";
    private static final String CHARACTER_ANIMATION_BACK = "ANIMATION_BACK";
    private static final String CHARACTER_ANIMATION_LEFT = "ANIMATION_LEFT";
    private static final String CHARACTER_ANIMATION_RIGHT = "ANIMATION_RIGHT";
    private static final String CHARACTER_CAMERA_BACK = "CAMERA_BACK";
    private static final String CHARACTER_CAMERA_LEFT = "CAMERA_LEFT";
    private static final String CHARACTER_CAMERA_RIGHT = "CAMERA_RIGHT";
    private static final String CHARACTER_SOUND_BOSS_BATTLE_MUSIC = "BOSS_BATTLE_MUSIC";
    private static final String CHARACTER_BEHAVIOR_BINDING_ANIMATION_PLAY = "ANIMATION_PLAY";
    private static final String CHARACTER_BEHAVIOR_BINDING_ANIMATION_STOP = "ANIMATION_STOP";
    private static final String CHARACTER_BEHAVIOR_BINDING_CAMERA_STATE = "CAMERA_STATE";
    private static final String CHARACTER_BEHAVIOR_BINDING_CAMERA_SWITCH = "CAMERA_SWITCH";
    private static final String CHARACTER_BEHAVIOR_BINDING_PLAY_BOSS_BATTLE_MUSIC = "PLAY_BOSS_BATTLE_MUSIC";

    CharcterEntityBuilder(GameObjectManager gameObjectManager, Rectangle screenBounds) throws TransientGameException {
        this.gameObject = gameObjectManager.createGameObject();
        this.screenBounds = screenBounds;
    }

    CharcterEntityBuilder generate() throws TransientGameException {
    ...
    private Map<String, List<Resource>> defineResources() throws TransientGameException {
        ...
        for (int i = 0; i < 8; i++) {
            resourceLibrary.add(resourceMap.get(CHARACTER_RESOURCES_BACK).get(i));
        }
        for (int i = 0; i < 8; i++) {
            resourceLibrary.add(resourceMap.get(CHARACTER_RESOURCES_LEFT).get(i));
        }
        for (int i = 0; i < 8; i++) {
            resourceLibrary.add(resourceMap.get(CHARACTER_RESOURCES_RIGHT).get(i));
        }
    ...
    private Map<String, AnimationSequence> defineAnimations() throws TransientGameException {
        // define animation
        animationMap.put(CHARACTER_ANIMATION_BACK, animationFactory.createAnimationSequence(100L));
        for (int i = 0; i < 8; i++) {
            animationMap.get(CHARACTER_ANIMATION_BACK).addCel(resourceMap.get(CHARACTER_RESOURCES_BACK).get(i).getId());
        }
        animationMap.put(CHARACTER_ANIMATION_LEFT, animationFactory.createAnimationSequence(100L));
        for (int i = 0; i < 8; i++) {
            animationMap.get(CHARACTER_ANIMATION_LEFT).addCel(resourceMap.get(CHARACTER_RESOURCES_LEFT).get(i).getId());
        }
        animationMap.put(CHARACTER_ANIMATION_RIGHT, animationFactory.createAnimationSequence(100L));
        for (int i = 0; i < 8; i++) {
            animationMap.get(CHARACTER_ANIMATION_RIGHT).addCel(resourceMap.get(CHARACTER_RESOURCES_RIGHT).get(i).getId());
        }

        // register animations
        gameObject.registerFeature(
            AnimationSequenceLibrary.create().add(
                animationMap.get(CHARACTER_ANIMATION_BACK)
            ).add(
                animationMap.get(CHARACTER_ANIMATION_LEFT)
            ).add(
                animationMap.get(CHARACTER_ANIMATION_RIGHT)
            )
        );
    ...
    private Map<String, Camera> defineCameras() throws TransientGameException {
        // define cameras
        cameraMap.put(CHARACTER_CAMERA_BACK, Camera.create(animationMap.get(CHARACTER_ANIMATION_BACK).getId(), AnimationSequence.class));
        cameraMap.put(CHARACTER_CAMERA_LEFT, Camera.create(animationMap.get(CHARACTER_ANIMATION_LEFT).getId(), AnimationSequence.class));
        cameraMap.put(CHARACTER_CAMERA_RIGHT, Camera.create(animationMap.get(CHARACTER_ANIMATION_RIGHT).getId(), AnimationSequence.class));

        // register cameras
        gameObject.registerFeature(
            CameraLibrary.create().add(
                cameraMap.get(CHARACTER_CAMERA_BACK)
            ).add(
                cameraMap.get(CHARACTER_CAMERA_LEFT)
            ).add(
                cameraMap.get(CHARACTER_CAMERA_RIGHT)
            )
        );
    ...
    private Map<String, SoundResource> defineSounds() throws TransientGameException {
        // define sounds
        soundMap.put(CHARACTER_SOUND_BOSS_BATTLE_MUSIC, resourceFactory.createSoundResource("zelda-boss-battle.ogg", 20.0f));

        // register sounds
        gameObject.registerFeature(
            SoundLibrary.create().add(
                soundMap.get(CHARACTER_SOUND_BOSS_BATTLE_MUSIC)
            )
        );
    ...
    private Map<String, BehaviorBinding> defineBehaviorBindings() throws TransientGameException {
        ...
        BehaviorParameterGeneratorDefinition cameraSwitchTargetSelector = (inputs) -> {
            ...
            String cameraId =
                orientation.get().equals("back") ?
                    cameraMap.get(CHARACTER_CAMERA_LEFT).getId() :
                    orientation.get().equals("left") ?
                        cameraMap.get(CHARACTER_CAMERA_RIGHT).getId() :
                        cameraMap.get(CHARACTER_CAMERA_BACK).getId();
        ...
        behaviorBindingMap.put(
            CHARACTER_BEHAVIOR_BINDING_ANIMATION_PLAY,
        ...
        behaviorBindingMap.put(
            CHARACTER_BEHAVIOR_BINDING_ANIMATION_STOP,
            BehaviorBinding.create(
        ...
        behaviorBindingMap.put(
            CHARACTER_BEHAVIOR_BINDING_CAMERA_SWITCH,
            BehaviorBinding.create(
                new Behavior(
        ...
        behaviorBindingMap.put(
            CHARACTER_BEHAVIOR_BINDING_PLAY_BOSS_BATTLE_MUSIC,
            BehaviorBinding.create(
                new Behavior(
                    BehaviorEnum.SOUND_PLAY,
                    ImmutableSet.of(BehaviorParameterFactory.getInstance().createIdParameter(soundMap.get(CHARACTER_SOUND_BOSS_BATTLE_MUSIC).getId()))
                ),
                HIDEventEnum.O,
                1000L
            )
        ...
        gameObject.registerFeature(
            BehaviorBindingLibrary.create().add(
                behaviorBindingMap.get(CHARACTER_BEHAVIOR_BINDING_ANIMATION_PLAY)
            ).add(
                behaviorBindingMap.get(CHARACTER_BEHAVIOR_BINDING_ANIMATION_STOP)
            ).add(
                behaviorBindingMap.get(CHARACTER_BEHAVIOR_BINDING_CAMERA_SWITCH)
            )
        );
